...
function renderBanners() {
  ...
  if (!state.dismissedBackup) {
    ...
    if (days === null || days >= 30) {
      const txt = days === null ? 'Progress is saved only on this device — back it up' : ('Last backup was ' + days + 'd ago — back up your progress');
...
function goalFor(filterId) {
  ...
  if (!label) { const f = filters.find(x => x.id === filterId); label = f ? f.label : 'Your plan'; }
...
function editPace2() {
  const v = prompt('Study hours per week you expect AFTER Sep 2026 (stored only in this browser):', state.pace2);
  if (v === null) return;
  const n = parseInt(String(v).replace(/[^0-9]/g, ''), 10);
  if (!isNaN(n) && n >= 0 && n <= 80) {
    state.pace2 = n;
...
function renderDashboard() {
  ...
  const moneyCard = `
      <div class="card">
        ...
        <div style="margin-top:10px;font-size:10px;color:var(--dim);text-align:center">Employer-funded: ${empPassed}/${empCerts.length} certs · £0 to you</div>
      </div>`;
  ...
  const tailCard = tail.length > 0 ? `
      <div class="card">
        ...
        <div style="font-size:11px;color:var(--muted);margin-bottom:6px"><strong>${tail.length} C/D-tier certs · ≈${tailHours}h</strong> — the bottom of the value ladder. Each bench pulls your pace outlook forward. Benching ≠ deleting; they stay in the catalogue.</div>
  ...
  if (remHours > 0) {
    ...
    projHTML = `<div class="roi-proj">⏳ ${remHours}h of study left in this scope · at 13h/wk now → <button class="roi-salary-edit" onclick="editPace2()" title="Edit post-Sep pace — stored only in your browser">${state.pace2}h/wk from Sep 2026 ✎</button> · credential-ready ≈ <strong>${projStr}</strong></div>`;
  }
  const gapHTML = goalMid ? `
      <div class="roi-gap">
        <div class="roi-gap-top">
          <button class="roi-salary-edit" onclick="editSalary()" title="Edit — stored only in your browser, never in the file">You're at £${(state.currentSalary || 0).toLocaleString()} ✎</button>
  ...
  const roleCard = `
    <div class="card role-card">
      ...
      <div class="roi-lede">Live ranking of which roles your passed certs point to — by share of each role's credentials earned (value-weighted). Re-ranks as you mark certs complete. Tap a role to optimise the card above for it. Credential coverage, not a job guarantee.</div>
      ${roleRows || '<div class="roi-empty">Mark some certs complete to see your closest roles.</div>'}
    </div>`;
...
function renderStrategy() {
  ...
  return `
    ...
    <div class="strategy-intro">
      ...
      <p class="strategy-lede">A structured certification plan organised in 6 phases. Certs are tagged across three career destination tracks — <strong>Physical Security Architect</strong> (B), <strong>Cyber Security Engineer/Architect</strong> (C), and <strong>Cloud Security Architect</strong> (A) — so you can filter by relevance to current direction. Foundation certs (A·B·C) serve all three tracks.</p>
    ...
    <details class="strategy-section track-section track-mypath-section" open>
      ...
      <div class="strategy-body">
        ...
        <div style="background: linear-gradient(135deg, rgba(62, 230, 160, 0.08), rgba(62, 230, 160, 0.04)); border-left: 3px solid #34d399; padding: 12px 14px; margin: 12px 0; border-radius: 6px; font-size: 11px; line-height: 1.6;">
          ...
          <p style="margin: 6px 0; color: #d4d1e8;">The path costs ~£32.5k of exam fees over the ~10-year horizon — but with the right approach, your <em>real</em> spend is far lower and your early return is far higher. Four levers:</p>
          ...
          <ul style="margin: 2px 0 6px 16px; padding: 0; color: #d4d1e8;">
            <li><strong>Employer-funded (£0 to you):</strong> LCA · LCP · LCDA · MCIT · MCIE · MCDE · ACP · IEC 62443-CFS — the relevant organisation pays for these as an Axis/Milestone/LenelS2 partner. Pure ROI: career value at zero personal cost. Knock these out first.</li>
          ...
          <p style="margin: 2px 0 0; color: #d4d1e8;"><strong>GICSP via SANS (~£7,800)</strong> is the single biggest line item. The <strong>exam-only challenge route (~£1,300)</strong> cuts it dramatically if you self-study with the ISA/IEC 62443 materials and your OT lab — strongly worth weighing before committing to the full SANS course. Saving here ≈ £6,500.</p>
        ...
        <div style="background: linear-gradient(135deg, rgba(85, 214, 255, 0.08), rgba(85, 214, 255, 0.04)); border-left: 3px solid #38bdf8; padding: 12px 14px; margin: 12px 0; border-radius: 6px; font-size: 11px; line-height: 1.6;">
          ...
          <p style="margin: 6px 0; color: #d4d1e8;">Of your five core languages, only two have certifications worth holding. The rest are validated through certs you already have — chasing a standalone cert for them would be the time-waste to avoid:</p>
          <ul style="margin: 4px 0 6px 16px; padding: 0; color: #d4d1e8;">
            ...
            <li><strong style="color:#a0a0c8;">SQL ❌</strong> — no security-relevant standalone cert. Your SQL (KQL + Splunk SPL) is validated via the Splunk certs + SC-200. A pure Oracle/DP-900 SQL cert is a data-admin credential — wrong signal, skip it.</li>
            <li><strong style="color:#a0a0c8;">Bash ❌</strong> — no standalone Bash cert exists; covered by <strong>Linux+ (XK0-006)</strong> if you want the credential (now part of My Path — add only if a Linux/cloud-heavy role demands it).</li>
          </ul>
        ...
        <div class="gateway-project">
          ...
          <ul style="margin:6px 0 0 0;padding-left:18px;font-size:11px;line-height:1.5">
            ...
            <li><strong>Senior (the differentiator):</strong> Build a converged physical + OT + cyber reference architecture for a fictional CNI site — VMS/access-control on the IT side, a 62443-zoned OT network, and the secured boundary between them. Document it as a full ADR (mapping to NCSC CAF + ISO 27001 + IEC 62443 + NIST AI RMF + SABSA attributes) and publish it. THIS is the portfolio piece that proves convergence design depth. <strong>If you pursue the geospatial track,</strong> extend it with a camera-analytics → ArcGIS <strong>common operating picture</strong> — a site map with live camera analytics plotted spatially, delivered as a customer-style web dashboard. That’s a differentiator almost no one else can demonstrate.</li>
    ...
    <details class="strategy-section" open>
      ...
      <div class="strategy-body">
        ...
        <p><strong>Vendor ladder note:</strong> Most vendor certs (LenelS2 LCA→LCP, Milestone MCIT→MCIE (capstone) + MCDE (design), Cisco Meraki CMSS) sit in Phase 1 because they're employer-funded and run continuously alongside the self-study spine — paced by customer project arrival rather than by your study schedule. The LenelS2 senior tier (LCE Phase 3, LCDA Phase 4) is gated by deployment-hour accumulation: LCE realistically lands when you've shadowed 2-3 OnGuard enterprise installs, LCDA lands when you've led one. Phase placement reflects eligibility, not study order.</p>
      </div>
    </details>
    ...
    <div style="background: linear-gradient(135deg, rgba(255, 201, 77, 0.07), rgba(255, 181, 77, 0.03)); border-left: 3px solid #ffc94d; padding: 10px 12px; margin: 10px 0; border-radius: 6px; font-size: 11px; line-height: 1.55;">
      ...
      <p style="margin: 6px 0 0; color: #d4d1e8;"><strong style="color:#ffc94d;">Held honestly:</strong> this is a <em>forward bet</em> — timing and mainstream adoption aren't guaranteed — so it's sized as a focused 5-cert layer (~£900), not a career-defining commitment. You're also pursuing it because it genuinely interests you, which is a perfectly good reason to hold a small, cheap, on-theme specialist track. It enriches the physical/CNI edge of the convergence profile; it never replaces the OT + cyber spine.</p>
    </div>
      ...
      <div class="strategy-body">
        ...
        <div class="why-natural"><strong>💡 Why natural for this path:</strong> You ALREADY do TAM-like work in your relevant hands-on experience — supporting customer deployments, troubleshooting configs, managing the vendor relationship. Pivoting to vendor-side TAM is the same job, different employer. Lower friction than SE (less sales pressure, similar pay).</div>
          ...
          <li data-rank="#4">
            ...
            <div class="why-natural"><strong>💡 Why natural for this path:</strong> You already do integration architecture — different scale, same skill class. Hyperscaler CSAs are typically pulled from sysadmin or solutions-engineer roles, not from pure cyber. Your "I can design and explain systems to customers" muscle is exactly what they hire for. The pay ceiling at AWS/Azure/GCP for senior CSAs significantly exceeds independent SE roles.</div>
          ...
          <li data-rank="#9">
            ...
            <div class="why-natural"><strong>💡 Why natural for this path:</strong> Heavy report writing — be honest about whether this energises or drains you. The path requires both technical depth AND business fluency, which is rare. Your integrator role already gives you "I've seen what breaks at customer scale" — exactly the muscle DD demands. But this is the longest arc (7-10 years) and the highest writing tolerance of any role we've discussed. The compensation ceiling justifies it only if the work itself genuinely interests you.</div>
    ...
    <div class="cert-details">
      ...
      ${cert.roles && cert.roles.length ? `
        <div class="cert-roles">
          <div class="cert-roles-label">🎯 Roles this helps you apply for</div>
