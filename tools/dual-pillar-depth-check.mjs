...
if(!recommendation.includes('skew'))errors.push('Recommendation engine does not retain numeric M/K imbalance information.');
...
const production=`${recommendation}\n${depth}\n${ui}`;
for(const legacy of ['DUAL-STRONG','MARKET-HEAVY','CAPABILITY-HEAVY'])if(production.includes(legacy))errors.push(`Illustrative M/K band ${legacy} has leaked back into production logic/UI.`);
if(/tandem\.label/.test(production))errors.push('Production still depends on a named tandem ranking label; use numeric M/K balance instead.');
if(!ui.includes('BAL ${p.tandem.balance}/10'))errors.push('Certification UI does not expose numeric M/K balance.');
...
console.log(`Dual-pillar depth gate passed: ${pvIds.length} explicit job pathways mapped; certification/pathway depth, numeric M/K balance and UI coverage enforced.`);
