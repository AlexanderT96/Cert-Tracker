// Cert Tracker — constraint-aware certification sequence planner.
  ...
  function plan(options={}) {
    ...
    for (let step=0;step<cfg.maxCerts;step++) {
      ...
      const ranked=feasible.map(cert=>{
        const rec=CT.recommendations.score(cert,{goal,passes});
    ...
    return Object.freeze({ goal,goalLabel:CT.competency.GOALS[goal]?.label||goal,settings:cfg,sequence:Object.freeze(chosen),usedHours:Math.round(usedHours),usedBudget:Math.round(usedBudget),baselineCoverage:baseline,projectedCoverage:projected,coverageGain:projected-baseline,blocked,withinTarget:usedHours<=maxHours&&usedBudget<=cfg.budget });
  }
