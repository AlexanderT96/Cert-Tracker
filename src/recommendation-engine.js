// Cert Tracker — explainable, competency-aware and role-aware recommendation engine.
(function initRecommendationEngine(global) {
  ...
  function horizonPoints(card,horizon='now'){
    const spec=HORIZONS[horizon]||HORIZONS.now;
    return Math.round(Object.entries(spec.weights).reduce((sum,[key,w])=>sum+Number(card?.[key]||0)*10*w,0));
  }
  ...
  function score(cert,options={}){
    ...
    const horizon=options.horizon||'now';
    const breakdown={
      track:TRACK_WEIGHT[cert.track]??0,
      roleValue:horizonPoints(career,horizon),
      timing:timingPoints(career.T),
      competency:Math.round(relevance.score*.55),
      readiness:Math.round(ready.score*.20),
      marketSignal:value?Math.min(12,Math.round(value.contributionRange.midpoint/900)):0,
      novelty:value?Math.round((value.novelty-50)/10):0,
      phase:phaseDistance===0?20:phaseDistance===1?8:phaseDistance<0?1:Math.max(-16,4-phaseDistance*5),
      path:inPath?18:-8, employer:cert.employer?10:0, gateway:cert.gateway?10:0,
      efficiency:hours<=20?12:hours<=40?9:hours<=70?6:hours<=110?2:-4,
    ...
    return Object.freeze({cert,id:cert.id,name:cert.name,score:Math.round(total),available:dep.satisfied&&!passes?.[cert.id]&&!state.skipped?.[cert.id],dependencies:dep,relevance,career,health,marketValue:value,readiness:ready,estimatedHours:hours,phaseDistance,effectivePhase,inPath,horizon,breakdown:Object.freeze(breakdown),reasons:Object.freeze(reasons)});
  }
