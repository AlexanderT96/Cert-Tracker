// Cert Tracker v3.1 — explainable, goal-aware career recommendation engine.
(function initRecommendationEngine(global) {
  ...
  const GOALS = Object.freeze({
    convergence: Object.freeze({ label: 'Convergence / OT Security Architect', keywords: ['axis','milestone','lenel','access control','video','vms','network','security','cloud','architecture','integration','ot','ics','palo alto','crowdstrike','arcgis','esri','claroty','nozomi'], roleTrackBonus: 14 }),
    cyber: Object.freeze({ label: 'Cyber Security Engineer', keywords: ['security','soc','siem','incident','threat','detection','iam','identity','zero trust','pentest','linux','python','crowdstrike','palo alto','isc2','comptia'], roleTrackBonus: 7 }),
    network: Object.freeze({ label: 'Network / Security Engineer', keywords: ['network','routing','switching','cisco','firewall','wireless','tcp','ip','subnet','palo alto','fortinet','wireshark'], roleTrackBonus: 8 }),
    physical: Object.freeze({ label: 'Physical Security Architect', keywords: ['axis','milestone','lenel','access control','video','camera','vms','intercom','physical security','integration','design'], roleTrackBonus: 18 }),
    cloud: Object.freeze({ label: 'Cloud Security Architect', keywords: ['cloud','azure','aws','iam','identity','security','architecture','zero trust','network','automation','devops'], roleTrackBonus: 6 })
  });

  const TRACK_WEIGHT = Object.freeze({ CORE: 30, FOUNDATION: 18, 'ROLE-DRIVEN': 25, ARCHITECT: 28, 'IDENTITY-SEC': 18, CONDITIONAL: 12, OPTIONAL: 4, 'POST-PLAN': -4 });
  ...
  function goalRelevance(cert, goalKey = currentGoal()) {
    ...
    const text = [cert?.name, cert?.vendor, cert?.coverage, cert?.note, ...(cert?.skills || []), ...(cert?.subjects || [])].filter(Boolean).join(' ').toLowerCase();
    ...
    return { matches, score: Math.min(30, matches.length * 4 + (specialistTrack ? profile.roleTrackBonus : 0)) };
  }
  ...
  function score(cert, options = {}) {
    ...
    const value = CT.marketValue?.contribution(cert) || null;
    ...
    const breakdown = {
      ...
      marketSignal: value ? Math.min(10, Math.round(value.contributionRange.midpoint / 1000)) : 0,
    ...
    let total = Object.values(breakdown).reduce((sum, n) => sum + n, 0);
    ...
    if (value?.contributionLabel) reasons.push(`Indicative career-value signal ${value.contributionLabel} (${value.confidence.toLowerCase()} confidence)`);
    ...
    return Object.freeze({ cert, id: cert.id, name: cert.name, score: Math.round(total), available: dep.satisfied && !state.passes?.[cert.id] && !state.skipped?.[cert.id], dependencies: dep, relevance, health, marketValue: value, estimatedHours: hours, phaseDistance, inPath, breakdown: Object.freeze(breakdown), reasons: Object.freeze(reasons) });
  }
  ...
  function recommend(options = {}) {
    ...
    return CERTS.filter(cert => !state.passes?.[cert.id] && !state.skipped?.[cert.id]).map(cert => score(cert, { goal })).filter(item => includeBlocked || item.available).sort((a, b) => b.score - a.score || a.estimatedHours - b.estimatedHours || a.name.localeCompare(b.name)).slice(0, limit);
  }
  function explainTop(options = {}) {
    ...
    const opportunityCost = runnerUp ? `Choosing ${top.name} first scores ${Math.max(0, top.score - runnerUp.score)} points higher than ${runnerUp.name} for the current goal profile.` : 'No other currently available certification outranks this choice.';
    return Object.freeze({ goal: currentGoal(), goalLabel: GOALS[currentGoal()].label, top, runnerUp, opportunityCost });
  }
  function scenario(goalKey) {
    if (!GOALS[goalKey]) throw new Error(`Unknown goal profile: ${goalKey}`);
    return { goal: goalKey, label: GOALS[goalKey].label, recommendations: recommend({ goal: goalKey, limit: 10 }) };
  }
