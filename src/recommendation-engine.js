...
(function initRecommendationEngine(global) {
  ...
  function currentGoal(){if(CT.focusedRoute?.enabled())return'network';const key=localStorage.getItem(CT.config.goalKey)||'convergence';return GOALS[key]?key:'convergence';}
  ...
  function recommend(options={}){
    if(CT.focusedRoute?.enabled()&&(!options.goal||options.goal===currentGoal())){
      const cert=CT.focusedRoute.next(options.passes||state.passes);if(!cert)return[];
      const item=score(cert,options),warning=state.skipped?.[cert.id]?'This milestone is marked skipped: review it before continuing.':!item.available?'Review prerequisites and availability before booking.':'Study this milestone before starting the next.';
      return [Object.freeze({...item,locked:true,portfolioClass:'LOCKED MILESTONE',reasons:Object.freeze(['Next in the locked route; one certification at a time.',warning,'Exam completion does not establish job readiness.'])})];
    }
    const limit=Number(options.limit||5);const includeBlocked=!!options.includeBlocked;const goal=options.goal||currentGoal();const passes=options.passes||state.passes;const horizon=options.horizon||'now';return CERTS.filter(cert=>!passes?.[cert.id]&&!state.skipped?.[cert.id]).map(cert=>score(cert,{goal,passes,horizon})).filter(item=>includeBlocked||item.available).sort((a,b)=>b.score-a.score||b.tandem.weaker-a.tandem.weaker||b.tandem.strength-a.tandem.strength||b.relevance.percent-a.relevance.percent||a.name.localeCompare(b.name)).slice(0,limit);
  }
