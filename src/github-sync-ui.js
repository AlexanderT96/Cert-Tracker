...
(function initGitHubSyncUI(global){
  ...
  function setLauncherState(){const b=document.getElementById('ct-github-sync-launcher');if(!b)return;b.classList.toggle('connected',CT.githubSync.isConnected());b.textContent=CT.githubSync.isConnected()?'Sync':'Sync';}
  function show(){
    const cfg=CT.githubSync.getConfig(),connected=CT.githubSync.isConnected();
    const body=`
      <div class="ct3-notice"><strong>Private GitHub sync</strong><br>Your tracker state is encrypted in this browser with AES-GCM before it is uploaded. The GitHub token and vault passphrase stay in memory for this browser session only and are never committed to either repository.</div>
      <div class="ct3-field"><label>Private repository (owner/repository)</label><input id="ct-gh-repo" value="${esc(cfg.repo)}" placeholder="your-account/private-repository" autocomplete="off"></div>
      <div class="ct3-grid">
      ...
      <div class="ct3-field"><label>Fine-grained GitHub token (session only)</label><input id="ct-gh-token" type="password" autocomplete="off" placeholder="Token scoped only to the private sync repository"></div>
      ...
      <div class="ct-gh-sync-help">For least privilege, use a <strong>fine-grained personal access token</strong> restricted to only the chosen private repository with <strong>Contents: Read and write</strong>. Do not grant Actions, Administration, Issues, Pull Requests, or access to other repositories just for tracker sync.</div>
    ...
    CT.ux.modal({title:'Private device sync',subtitle:'Encrypted state sync through your own private GitHub repository',body,onMount(root){
      ...
      const connect=()=>{saveCfg();CT.githubSync.connect({token:root.querySelector('#ct-gh-token').value,passphrase:root.querySelector('#ct-gh-passphrase').value});setLauncherState();setStatus('Session connected. Token and passphrase remain in memory only.');};
      const ensure=()=>{if(!CT.githubSync.isConnected())connect();else saveCfg();};
      root.querySelector('#ct-gh-connect').addEventListener('click',()=>{try{connect();}catch(e){setStatus(e.message);}});
      root.querySelector('#ct-gh-test').addEventListener('click',async()=>{try{ensure();setStatus('Checking private repository access…');const r=await CT.githubSync.testConnection();setStatus(`Access confirmed: ${r.repo} · ${r.branch} · ${r.path}`);}catch(e){setStatus(e.message);}});
      ...
      root.querySelector('#ct-gh-disconnect').addEventListener('click',()=>{CT.githubSync.disconnect();root.querySelector('#ct-gh-token').value='';root.querySelector('#ct-gh-passphrase').value='';setLauncherState();setStatus('Session secrets forgotten. Repository/path settings remain local to this browser.');});
    }});
  }
