...
(function initCareerFramework(global) {
  ...
  function setContext(update={}) {for (const [field,value] of Object.entries(update)) {const storageKey = STORAGE[field];if (!storageKey) continue;if (!ROLE_PROFILES[value]) throw new Error(`Unknown role profile: ${value}`);localStorage.setItem(storageKey,value);}CT.events.emit('career-context-changed', context());return context();}
