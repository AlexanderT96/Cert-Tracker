...
(function initUX(global) {
  ...
  function showDataHealth() {
    ...
    const body = `<div data-cert-data-health><div class="ct3-grid"><div class="ct3-card"><h3>Confidence</h3><div class="ct3-big">${s.averageConfidence}%</div><div class="ct3-muted">Source-confidence score across ${s.total} records</div></div><div class="ct3-card"><h3>Source coverage</h3><div class="ct3-big">${s.sourceCoverage}%</div><div class="ct3-muted">${s.total - s.missingSources}/${s.total} linked · ${s.exactSources} cert-level · ${s.vendorSources} vendor-level</div></div></div><div class="ct3-notice" style="margin-top:12px">Coverage means an official source is linked, not that every detail is verified. Confidence is a heuristic based on provenance and freshness, not a guarantee of current prices, exam availability or career outcomes. Cert-level sources identify the credential; vendor-level sources still need exact-track verification. Source checks do not refresh price-verification dates.</div><div class="ct3-card" style="margin-top:12px"><h3>Review queue · ${queue.length}</h3><div class="ct3-muted">${s.availabilityWarnings} credential-availability warnings</div>${queue.map(row => `<div class="ct3-row"><div><strong>${esc(row.name)}</strong><div class="ct3-muted">${esc(row.issues.join(' · '))}</div>${row.sourceNote ? `<div class="ct3-muted">${esc(row.sourceNote)}</div>` : ''}<div class="ct3-muted">Source checked: ${esc(row.sourceCheckedAt || 'not independently checked')} · Record dated: ${esc(row.verifiedAt || 'unknown')}</div>${row.sourceUrl ? `<a class="ct3-link" href="${esc(row.sourceUrl)}" target="_blank" rel="noopener">Official source</a>` : ''}</div><span class="ct3-pill">${esc(row.freshness)} · ${row.confidence}%</span></div>`).join('') || '<div class="ct3-muted">No records require review.</div>'}</div><div class="ct3-actions"><button class="ct3-btn" id="ct3-health-export">Export audit CSV</button></div></div>`;
