...
(function initMarketReadiness(global){
  ...
  const GENERIC_FILTERS=new Set(['my-path','all','passed']);
  ...
  function progress(cert){if(CT.credentials?.active(cert))return 1;const objective=clamp(CT.store.objective?.(cert.id)||0)/100;return objective;}
  ...
  function practicalScore(archetype){const keys=PILLAR_BY_ARCHETYPE[archetype]||[],records=keys.flatMap(k=>CT.capabilityGates.evidenceForPillar(k));return clamp(avg(records.map(r=>r.record.score)));}
  function experienceFit(){return 0;}
  function marketForPath(path){return Object.freeze({...BASE_MARKETS[path.spec.archetype],...(PATHWAY_MARKETS[path.id]||{})});}
  function readinessLabel(score){if(score>=72)return'COMPARE EVIDENCE';if(score>=45)return'BUILD NEXT';return'NOT ASSESSED / BUILD';}
  function careerRow(role){const a=CT.careerOptions.assess(role),archetype=CT.careerOptions.FAMILIES[role.family].archetype,market={...(BASE_MARKETS[archetype]||BASE_MARKETS.architectureConsulting),confidence:'UNVERIFIED MODEL',query:role.title};return Object.freeze({id:role.id,label:role.title,archetype,marketAccess:a.credential,capability:a.readiness??0,score:a.readiness??0,status:a.status,compatibility:a.compatibility,assessment:a,market,query:role.title,missing:a.gaps[0]?.label||'Compare with a specific vacancy; eligibility is not established.'});}
  function roleRowFromPath(path){const canonical=CT.careerOptions?.byId(path.id);if(canonical)return {...careerRow(canonical),path};const marketCredential=pathwayCertScore(path,'market'),knowledgeCredential=pathwayCertScore(path,'knowledge'),practical=practicalScore(path.spec.archetype);const capability=clamp(knowledgeCredential*.58+practical*.42),marketAccess=clamp(marketCredential*.82+Math.min(100,path.metrics.market*10)*.18);const floor=Math.min(marketAccess,capability),score=clamp(floor*.72+(marketAccess+capability)/2*.28),market=marketForPath(path);return Object.freeze({id:path.id,label:path.label,archetype:path.spec.archetype,marketAccess:Math.round(marketAccess),capability:Math.round(capability),score:Math.round(score),status:practical?readinessLabel(score):'NOT ASSESSED',market,query:market.query||path.label,missing:marketAccess>capability?'Job-performance capability is the limiting pillar.':'Market access / recognised signal is the limiting pillar.',path});}
  function baselineRow(spec){const certs=CERTS.map(cert=>({cert,fit:CT.careerFramework.profileFit(cert,spec.roleKey),p:progress(cert)})).filter(x=>x.fit>=25&&x.p>0);const marketAccess=clamp(avg(certs.map(x=>x.fit*x.p))*.1+certs.reduce((s,x)=>s+CT.careerFramework.scoreCard(x.cert).M*x.p,0)*2.1);const practical=practicalScore(spec.archetype),experience=experienceFit(spec.roleKey),knowledge=clamp(avg(certs.map(x=>CT.careerFramework.scoreCard(x.cert).K*x.p))*10);const capability=clamp(knowledge*.4+practical*.35+experience*.25);const score=clamp(Math.min(marketAccess,capability)*.7+(marketAccess+capability)/2*.3);return Object.freeze({...spec,marketAccess:Math.round(marketAccess),capability:Math.round(capability),score:Math.round(score),status:practical?readinessLabel(score):'NOT ASSESSED',query:spec.market.query,missing:marketAccess>capability?'Capability evidence/experience is limiting readiness.':'Recognised role signal is limiting readiness.'});}
  function roles(){if(CT.careerOptions)return Object.freeze(CT.careerOptions.ROLES.map(careerRow).sort((a,b)=>b.compatibility-a.compatibility||b.score-a.score));const paths=CT.dualPillarDepth.allPathways().filter(p=>p.id.startsWith('pv-')&&!CT.careerOptions?.CONTEXTS[p.id]).map(roleRowFromPath),baseline=BASELINE_ROLES.map(baselineRow);const byLabel=new Map();for(const row of [...baseline,...paths]){const key=row.label.toLowerCase(),prior=byLabel.get(key);if(!prior||row.score>prior.score)byLabel.set(key,row);}return Object.freeze([...byLabel.values()].sort((a,b)=>b.score-a.score||b.market.median-a.market.median));}
  ...
  function currentValue(){const rows=roles(),apply=rows.filter(r=>r.status==='Compare vacancies'),anchor=apply[0]||rows[0]||null;if(!anchor)return Object.freeze({low:0,high:0,anchor:null,roles:rows});const top=apply.slice(0,5),ranges=(top.length?top:[anchor]).map(adjustedRange),low=Math.min(...ranges.map(r=>r.low)),high=Math.max(...ranges.map(r=>r.high));return Object.freeze({low,high,anchor,roles:rows,applyNow:Object.freeze(rows.filter(r=>r.status==='Compare vacancies').slice(0,6)),stretch:Object.freeze(rows.filter(r=>r.score>=58&&r.score<72).slice(0,6)),next:Object.freeze(rows.filter(r=>r.score>=45&&r.score<58).slice(0,6)),methodology:SNAPSHOT.methodology});}

  function filterItem(filterId=state.filter){return CT.dualPillarDepth.filterItems().find(x=>x.id===filterId)||null;}
  function isRoleFocusedFilter(filterId=state.filter){const item=filterItem(filterId);return !!item&&!CT.careerOptions?.CONTEXTS[item.id]&&!GENERIC_FILTERS.has(item.id)&&!item.id.startsWith('group-');}
  function pathwayProgress(path){const certs=path.members.map(id=>CERTS.find(c=>c.id===id)).filter(Boolean),passed=certs.filter(c=>state.passes?.[c.id]),inProgress=certs.filter(c=>!state.passes?.[c.id]&&progress(c)>0),unstarted=certs.filter(c=>!state.passes?.[c.id]&&progress(c)===0);const weighted=certs.length?Math.round(certs.reduce((s,c)=>s+progress(c),0)/certs.length*100):0;return Object.freeze({total:certs.length,passed:passed.length,inProgress:inProgress.length,unstarted:unstarted.length,weighted,passedIds:Object.freeze(passed.map(c=>c.id)),inProgressIds:Object.freeze(inProgress.map(c=>c.id))});}
  function nextCertGaps(path,limit=6){const certs=path.members.map(id=>CERTS.find(c=>c.id===id)).filter(c=>c&&!state.passes?.[c.id]);const ranked=CT.filterIntelligence?.rankRows?CT.filterIntelligence.rankRows(certs,{filterId:path.id,label:path.label,horizon:'now'}):certs.map(cert=>CT.recommendations.score(cert,{horizon:'now'}));return Object.freeze(ranked.slice(0,limit).map(item=>{const cert=item.cert||CERTS.find(c=>c.id===item.id),card=item.career||CT.careerFramework.scoreCard(cert),tandem=item.tandem||CT.recommendations.tandemProfile(card);return Object.freeze({id:cert.id,name:cert.name,code:cert.code||'',M:card.M,K:card.K,timing:card.T,progress:Math.round(progress(cert)*100),available:item.available!==false,tandem:tandem.label,reason:(item.reasons||[])[0]||''});}));}
  function evidenceGaps(path,limit=6){const canonical=CT.careerOptions?.byId(path.id);if(canonical)return Object.freeze(CT.careerOptions.assess(canonical).gaps.slice(0,limit).map(q=>({id:q.id,pillar:canonical.family,label:canonical.title,evidence:q.label,level:q.level,score:q.coverage})));const pillars=PILLAR_BY_ARCHETYPE[path.spec.archetype]||[],rows=pillars.flatMap(pillar=>CT.capabilityGates.evidenceForPillar(pillar).map(item=>({pillar,label:CT.capabilityGates.PILLARS[pillar]?.label||pillar,id:item.id,evidence:item.label,level:item.record.level,score:item.record.score})));const seen=new Set();return Object.freeze(rows.filter(row=>!seen.has(row.id)&&(seen.add(row.id),true)).filter(row=>row.score<80).sort((a,b)=>a.score-b.score||a.evidence.localeCompare(b.evidence)).slice(0,limit).map(Object.freeze));}
  function activeAssessment(filterId=state.filter){if(!isRoleFocusedFilter(filterId))return null;const item=filterItem(filterId),path=CT.dualPillarDepth.pathwayProfile(item),role=roleRowFromPath(path),progressSummary=pathwayProgress(path),range=adjustedRange(role),certGaps=nextCertGaps(path),evidence=evidenceGaps(path),floor=Math.min(role.marketAccess,role.capability);return Object.freeze({filterId:item.id,label:path.label,path,role,range,progress:progressSummary,nextCerts:certGaps,evidenceGaps:evidence,marketAccess:role.marketAccess,capability:role.capability,pillarFloor:floor,score:role.score,status:role.status,market:role.market,query:role.query,responsibilities:path.responsibilities,evidenceStandard:path.evidence,limitingPillar:role.marketAccess>role.capability?'capability':'market',limitingMessage:role.missing,applicationGuidance:'Compare demonstrated evidence with a specific vacancy. These heuristic scores do not establish hiring probability, eligibility or salary.'});}
  ...
  CT.marketReadiness=Object.freeze({SNAPSHOT,BASE_MARKETS,PATHWAY_MARKETS,BASELINE_ROLES,PILLAR_BY_ARCHETYPE,progress,certContribution,pathwayCertScore,practicalScore,roleRowFromPath,baselineRow,roles,currentValue,adjustedRange,readinessLabel,filterItem,isRoleFocusedFilter,pathwayProgress,nextCertGaps,evidenceGaps,activeAssessment,money});
})(window);
