// Cert Tracker v3 — provenance/freshness health for certification data.
(function initDataHealth(global) {
  ...
  function record(cert) {
    const verified = normaliseVerifiedDate(cert?.verifiedAt);
    const ageDays = verified ? Math.max(0, Math.floor((Date.now() - verified.getTime()) / 86400000)) : null;
    const exactSource = cert?.sourceUrl || cert?.officialUrl || null;
    const fallbackSource = vendorSource(cert);
    const sourceUrl = exactSource || fallbackSource;
    const sourceLevel = exactSource ? 'CERT' : fallbackSource ? 'VENDOR' : 'NONE';
    ...
    const issues = [];
    if (!cert?.verifiedAt) issues.push('No verification date');
    ...
    else if (sourceLevel === 'VENDOR') issues.push('Only vendor-level source mapped');
  ...
  function reviewQueue() {
    ...
    return allRecords()
      ...
      .sort((a, b) => order[a.freshness] - order[b.freshness] || a.confidence - b.confidence || a.name.localeCompare(b.name));
  }

  function toCsv() {
    const esc = value => `"${String(value ?? '').replace(/"/g, '""')}"`;
    const rows = [['id', 'name', 'verifiedAt', 'freshness', 'sourceLevel', 'sourceUrl', 'confidence', 'issues']];
    allRecords().forEach(row => rows.push([
      row.id, row.name, row.verifiedAt, row.freshness, row.sourceLevel, row.sourceUrl, row.confidence, row.issues.join('; ')
    ]));
