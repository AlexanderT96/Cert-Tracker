...
(function initRoleReadinessUI(global){
  ...
  function career(){
    ...
    const ranked=CT.roleReadiness.all().filter(row=>String(row.role.id||'').startsWith('pv-')).slice(0,8);
    const card=document.createElement('div');card.className='ct3-card';card.dataset.rankCareerCard='1';card.style.marginTop='12px';
    card.innerHTML=`<h3>Role readiness rank</h3><div class="ct3-muted" style="margin-bottom:8px">Six evidence-gated levels are calculated independently for every role pathway: Beginner / Training → Intermediate → Senior → Manager / Team Leader → Architect → Director. The same market-access and job-performance pillars apply at every level; higher ranks additionally require USED, DESIGNED and OWNED evidence.</div>${ranked.map(({role,rank})=>`<div class="ct-rank-career-row"><div><strong>${esc(role.label)}</strong><small>${esc(rank.title)} · readiness ${rank.score}% · M/K floor ${rank.floor}%${rank.next?` · next blocker: ${esc(rank.next.gaps[0]||rank.next.label)}`:''}</small></div><span class="ct-rank-career-badge">${esc(rank.key)} ${esc(rank.short)}</span></div>`).join('')}`;
