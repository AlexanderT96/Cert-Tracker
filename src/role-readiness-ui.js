...
(function initRoleReadinessUI(global){
  ...
  function style(){if(document.getElementById('ct-role-readiness-style'))return;const s=document.createElement('style');s.id='ct-role-readiness-style';s.textContent=`
    ...
    @media(max-width:700px){.ct-rank-dashboard-head{grid-template-columns:1fr}.ct-rank-dashboard-score{width:max-content}.ct-rank-dashboard-ladder{display:flex;overflow-x:auto;scrollbar-width:none}.ct-rank-dashboard-ladder::-webkit-scrollbar{display:none}.ct-rank-dashboard-step{flex:0 0 82px}.ct-rank-inline{margin-left:0;margin-top:4px}.ct-rank-all-body{max-height:55vh}}
  ...
  function careerRow({role,rank}){return `<div class="ct-rank-career-row"><div><strong>${esc(role.label)}</strong><small>${esc(rank.title)} · readiness ${rank.score}% · M/K floor ${rank.floor}%${rank.next?` · next blocker: ${esc(rank.next.gaps[0]||rank.next.label)}`:''}</small></div><span class="ct-rank-career-badge">${esc(rank.key)} ${esc(rank.short)}</span></div>`;}
  function career(){
    ...
    card.innerHTML=`<h3>Role readiness rank</h3><div class="ct3-muted" style="margin-bottom:8px">Six evidence-gated levels are calculated independently for every role pathway: Beginner / Training → Intermediate → Senior → Manager / Team Leader → Architect → Director. The same market-access and job-performance pillars apply at every level; higher ranks additionally require USED, DESIGNED and OWNED evidence.</div><div class="ct-dual-kicker" style="margin:8px 0 2px">STRONGEST CURRENT ROLE PATHWAYS</div>${strongest.map(careerRow).join('')}<details class="ct-rank-all"><summary>VIEW ALL ${ranked.length} ROLE PATHWAY RANKS</summary><div class="ct-rank-all-body">${ranked.map(careerRow).join('')}</div></details>`;
