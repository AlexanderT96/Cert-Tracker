...
(function initJobMarket(global){
  ...
  function rankJobs(feed,roles=CT.marketReadiness.roles()){
    return Object.freeze((feed?.jobs||[]).map(job=>{
  ...
  function liveBand(role,feed=cache){if(!feed||feed.status!=='live'||feed.refreshError||!Number.isFinite(Date.parse(feed.fetchedAt))||Date.now()-Date.parse(feed.fetchedAt)>900000)return null;const rows=rankJobs(feed,[role]).filter(x=>x.roleMatch>=38&&x.seniorityGap<=0).map(x=>x.salaryMid).filter(x=>x>=20000&&x<=250000).sort((a,b)=>a-b);if(rows.length<3)return null;const pct=p=>rows[Math.min(rows.length-1,Math.floor((rows.length-1)*p))];return Object.freeze({low:Math.round(pct(.2)/500)*500,median:Math.round(pct(.5)/500)*500,high:Math.round(pct(.8)/500)*500,samples:rows.length,source:feed?.provider||'live feed',fetchedAt:feed?.fetchedAt||null});}
